...
# ── VIIRS DNB 2013–2023 ───────────────────────────────────────────────────────
# Use VNL V2 annual composites instead of monthly — more stable band naming
# Collection: NOAA/VIIRS/DNB/ANNUAL_V21
# Band: average (not avg_rad — that is the monthly product band name)

VIIRS_YEARS = list(range(2013, 2024))

def extract_viirs_year(year):
    # Annual VNL V2.1 composite — one image per year, band = 'average'
    ...
    def reduce_city(feat):
        ...
        return feat.set('ntl_mean',   stats.get('average')) \
                   .set('year',       year) \
                   .set('ntl_source', 'VIIRS_VNL21')

    return city_fc.map(reduce_city)

print("Building VIIRS VNL V2.1 annual collections (2013-2023)...")
...
viirs_task = ee.batch.Export.table.toDrive(
    ...
    description    = 'gulf_ntl_viirs_vnl21_2013_2023',
    folder         = DRIVE_DIR,
    fileNamePrefix = 'gulf_ntl_viirs_vnl21_2013_2023',
    fileFormat     = 'CSV'
)
viirs_task.start()
print(f"  VIIRS VNL21 task submitted: {viirs_task.id}")

print("\nMonitoring (Ctrl+C to stop)...\n")
try:
    while True:
        state = viirs_task.status()['state']
        print(f"  VIIRS VNL21: {state}")
        if state in ('COMPLETED', 'FAILED', 'CANCELLED'):
            if state == 'FAILED':
                print("  Error:", viirs_task.status().get('error_message'))
            break
        print("  (waiting 30s...)\n")
        time.sleep(30)
